...
// salidaExamen();
//----Author APRENTEC -------
//----Servicios en Aprendizaje y Tecnología ..............
//----Fecha Creado: Febrero 2007......
// Fecha ultima actualización: 06 de mayo de 2007
...
function codigoExamen(nombreE) {
	...
	_global.currentQ = 0;			// nro de pregunta actual
...
function ConstruyeArregloPreguntas() {
	...
	for (var i = 0; i<nrofeedback; i++) {
		...
		final[i] = thisChoice.firstChild.nodeValue;
	}
...
function makeQuestion(currentQ, nropregunta) {
	...
	pregunta = _global.currentQ + 1
	enunciado_mc.qNum = "Pregunta "+(pregunta)+" de "+nroaMostrar;
...
_global.nextQ = function() {
	if ((_global.currentQ)+1 == nroaMostrar) {
		if (tipoExamen == "gimnasio") {
			mala.removeMovieClip();
			buena.removeMovieClip();
		}
		...
		_global.currentQ = _global.currentQ+1;
		makeQuestion(_global.currentQ, nropregunta);
	}
};
...
function resultadoFinal() {
	...
	_level0.puntaje= valorporcentaje;
	fscommand ("LMSSetValue","cmi.core.score.raw,"+valorporcentaje);
    Lstatus = "completed";
...
_global.salidaExamen = function(leccion, nrotemas, salida) {
	unloadMovieNum(1);
	direccion = "";
	tiempo = calcularTiempo();
	if (_level0.tipo == "registro") {
		if ((_level1.es<>"gimnasio") and (_level1.es<>"examenfinal")) {
			if (_level1.es == "trivia") {
				actual = "Trivia";
			} else {
				if (leccion == 0) {
					actual = "L"+_level1.modulo+"3";
				} else {
					actual = "L"+_level1.modulo+leccion;
				}
			}
			direccion = "../guardaReg.asp?tiempo="+tiempo+"&actual="+actual;
		} else if (_global.llegoalfinal) {
			if (es == "gimnasio") {
				actual = "Examen: "+_global.titulo;
			} else {
				actual = "Examenfinal";
			}
			score = _level1.valor;
			direccion = "../guardaExamen.asp?tiempo="+tiempo+"&score="+score+"&actual="+actual;
		}
		if (direccion<>"") {
			fscommand("IraPagina", direccion);
		}
	...
	if (leccion<>0) {
		leccion = Number(leccion)+1;
	}
	tiempototal = int(getTimer()/1000);
	if ((es<>"examenfinal") and (es<>"trivia")) {
		archivo = "../L"+_level1.modulo+leccion+"/L"+_level1.modulo+leccion+_level1.abreviatura+".swf?modulo="+_level1.modulo+"&leccion="+leccion+"&abreviatura="+_level1.abreviatura+"&totaltemas="+nrotemas+"&tiempo="+tiempototal+"&ultimo=1";
	} else {
		archivo = "../L"+_level1.modulo+leccion+"/L"+_level1.modulo+leccion+_level1.abreviatura+".swf?modulo="+_level1.modulo+"&leccion="+leccion+"&abreviatura="+_level1.abreviatura+"&totaltemas="+nrotemas+"&tiempo="+tiempototal+"&ultimo=1";
	}
	if (salida<>"salida") {
